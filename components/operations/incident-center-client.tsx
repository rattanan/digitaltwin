...
export function IncidentCenterClient({ initialData, canManage }: { initialData: IncidentOverview; canManage: boolean }) {
  ...
  const effectiveSelectedId = visibleItems.some((item) => item.id === selectedId) ? selectedId : visibleItems[0]?.id ?? "";
