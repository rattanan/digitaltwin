    ...
    <div className="mx-auto max-w-[1800px] space-y-5">
      ...
      <section className="grid gap-5 xl:grid-cols-[minmax(0,1.6fr)_minmax(320px,0.7fr)]">
        <Card className="overflow-hidden">
          <CardHeader className="flex-row items-start justify-between"><div><CardTitle className="flex items-center gap-2 text-base"><MapPin className="size-4 text-cyan-200" />แผนที่สถานการณ์จังหวัด</CardTitle><p className="mt-1 text-xs text-slate-500">ตัวอย่างภาพรวมพื้นที่และจุดข้อมูลสำคัญ · เปิดแผนที่ GIS เพื่อดูรายละเอียด</p></div><Button asChild variant="ghost" size="sm" className="shrink-0 text-cyan-200"><Link href="/map">เปิดแผนที่เต็ม <ArrowUpRight className="size-3.5" /></Link></Button></CardHeader>
        ...
        <div className="space-y-5">
          <Card><CardHeader className="flex-row items-center justify-between"><CardTitle className="flex items-center gap-2 text-base"><AlertTriangle className="size-4 text-amber-200" />แจ้งเตือนสำคัญ</CardTitle><Badge variant="danger">{activeAlerts} รายการ</Badge></CardHeader><CardContent className="space-y-3">{summary.news.map((item) => <div key={item.title} className="flex gap-3 rounded-xl border border-white/[0.07] bg-white/[0.025] p-3"><div className={`mt-1 size-2 shrink-0 rounded-full ${item.severity === "HIGH" ? "bg-rose-300" : item.severity === "WARNING" ? "bg-amber-300" : "bg-cyan-300"}`} /><div className="min-w-0"><p className="text-xs font-medium leading-5 text-slate-200">{item.title}</p><p className="mt-1 text-[10px] text-slate-600">{item.time}</p></div></div>)}<Button asChild variant="ghost" size="sm" className="w-full justify-center text-cyan-200"><Link href="/alerts">ดูศูนย์แจ้งเตือน <ChevronDown className="size-3.5 -rotate-90" /></Link></Button></CardContent></Card>
