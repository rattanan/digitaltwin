...
function DetailPanel({ detail, canManage, onStatusSave, onDelete }: { detail: IotDetail; canManage: boolean; onStatusSave: (status: IotStatus) => Promise<void>; onDelete: () => Promise<void> }) {
  const [nextStatus, setNextStatus] = useState<IotStatus>(detail.status);
  ...
  async function removeDevice() {
    if (!window.confirm(`ยืนยันการซ่อน ${detail.deviceCode} จากระบบหรือไม่`)) return;
    setRemoving(true);
    setActionError("");
    try { await onDelete(); } catch (error) { setActionError(error instanceof Error ? error.message : "ซ่อนอุปกรณ์ไม่สำเร็จ"); } finally { setRemoving(false); }
  }
  ...
  return <div className="space-y-5">
    ...
    {canManage && <Card className="border-emerald-200/10"><CardHeader className="pb-3"><CardTitle className="text-sm">การจัดการอุปกรณ์</CardTitle></CardHeader><CardContent className="space-y-3"><div className="flex gap-2"><Select className="min-w-0 flex-1" value={nextStatus} onChange={(event) => setNextStatus(event.target.value as IotStatus)} aria-label="สถานะอุปกรณ์"><option value="ONLINE">ออนไลน์</option><option value="OFFLINE">ออฟไลน์</option><option value="MAINTENANCE">ซ่อมบำรุง</option><option value="DEGRADED">คุณภาพลดลง</option></Select><Button size="sm" onClick={() => void saveStatus()} disabled={nextStatus === detail.status || saving}><Check className="size-3.5" />{saving ? "กำลังบันทึก" : "บันทึก"}</Button></div>{actionError && <p role="alert" className="text-xs text-rose-200">{actionError}</p>}<Button variant="danger" size="sm" className="w-full" onClick={() => void removeDevice()} disabled={removing}><Trash2 className="size-3.5" />{removing ? "กำลังดำเนินการ" : "ซ่อนอุปกรณ์จากรายการ"}</Button></CardContent></Card>}
  </div>;
}
...
export function IotClient({ initialData, canManage }: { initialData: IotOverview; canManage: boolean }) {
  ...
  async function updateStatus(status: IotStatus) {
    if (!detail) return;
    const updated = await api<IotDetail>(`/api/v1/iot/${detail.id}`, { method: "PATCH", body: JSON.stringify({ status }) });
    setDetail(updated);
